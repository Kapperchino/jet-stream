...
func (suite *ClientTestOneNodeCluster) SetupSuite() {
	...
	go factory.SetupServer(suite.address[0], testData, raftDir, suite.address[0], suite.nodeName[0], suite.gossipAddress[0], "", suite.servers, "shardA")
	time.Sleep(5 * time.Second)
	go factory.SetupServer(suite.address[1], testData, raftDir, suite.address[1], suite.nodeName[1], suite.gossipAddress[1], suite.gossipAddress[0], suite.servers, "shardA")
	time.Sleep(5 * time.Second)
	go factory.SetupServer(suite.address[2], testData, raftDir, suite.address[2], suite.nodeName[2], suite.gossipAddress[2], suite.gossipAddress[0], suite.servers, "shardA")
	time.Sleep(5 * time.Second)
